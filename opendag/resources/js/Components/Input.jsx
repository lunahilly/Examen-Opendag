const InputField = ({label, value, onChange, type, error, onClick, children, required, readOnly}) => {
    return(
        <div className="input">
            <label className={required ? 'input__label input__label--required' : 'input__label'} data-end={required ? ' *' : null}>{label}</label>
            <span className="input__wrapper">
                <input readOnly={readOnly} type={type != null ? type : 'text'} value={value} onChange={onChange} className="input__field" />
                {
